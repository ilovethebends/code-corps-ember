...
test('A github repo can be assigned to the task during creation', function(assert) {
  ...
  let { githubRepo } = server.create('project-github-repo', { project });
  ...
  andThen(() => {
    ...
    projectTasksNewPage.repoSelect.openDropdown();
  });

  andThen(() => {
    projectTasksNewPage.repoSelect.githubRepos(0).select();
  });
